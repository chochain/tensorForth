...
.( verify { { 6 6 } { 15 15 } } => ) . \ print the resultant matrix
...
: mx                         \ create a word for benchmark loops
  ...
  . ."  msec/cycle" cr ;     \ print result  (GTX1660 0.768 msec/cycle)
see mx                       \ show the word
0 trace
999 mx                       \ run the multiplication loop 1000 times
1 trace
2drop                        \ drop the original two matrices
...
+=                           \ add matrices (destructive)
.( {{1 2 3}{4 5 6}} += 1 )
.( = {{2 3 4}{5 6 7}} => ) . \ and print
...
3 2 matrix ones              \ create a 3x2 matrix filled with 1s
@= dup                       \ multiply (destructive)
.( { { 1 2 3 } { 0 4 5 } } @ { { 1 1 } { 1 1 } { 1 1 } } )
.( verify = { { 6 6 } { 9 9 } } => ) .  \ and print

cr .( ### matrix element-wise multiplication ) cr
2 2 matrix ones              \ create a matrix filled with 1s
0.5 *=                       \ matrix-scalar multiply (scale down)
*=                           \ matrix-matrix multiply (Hadamard)
.( { {6 6} {9 9} } * { { 0.5 0.5 } { 0.5 0.5 } } )
.( verify = { { 3 3 } { 4.5 4.5 } } => ) .

...
Corpus *Cifar10::init(U32 mini_bsz, bool trace) {
...
U32 Cifar10::fetch(U32 bid, bool trace) {
    ...
    ///
    U32 n = batch_sz = _get_data(bid, trace);   ///< load, update actual size loaded
...
U32 Cifar10::_get_data(U32 bid, bool trace) {
    ...
    U32 n = cnt / SAMPLE_BSZ;                      ///< actual samples loaded
    
    U8 *bp = buf.data(), *tp = label, *dp = data;
    for (U32 i = 0; i < n; i++) {
        *tp++ = *bp;                               /// * read label
        bp += LABEL_BSZ;
        for (U32 j = 0; j < IMAGE_HW; j++) {       /// * image
            *dp++ = *(bp + j);                     /// * R
            *dp++ = *(bp + IMAGE_HW + j);          /// * G

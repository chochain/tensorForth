...
TensorVM::process(char *idiom) {
    ...
    if (compile) {                            /// * add literal when in compile mode
        ...
        VLOG("vm%d> SP=%d ss.push(%g(0x%08x))\n", id, SP, n, DU2X(n));
        PUSH(n);
    }
    return 1;
}
...
TensorVM::_pickle(bool load, bool png) {
    ...
    if (SP > 1 && IS_OBJ(ss[-2])) { /* OK */ }
    else if (SP > 2 && IS_OBJ(ss[-3])) mode = POPi;
...
TensorVM::init() {
    ...
    CODE("gemm3",     gemm(3));               ///< (a b A B C -- a b A B C O) GEMM k_gemm_tile_claude (tiled/reg)
    CODE("gemm4",     gemm(4));               ///< (a b A B C -- a b A B C O) GEMM k_gemm_tile_claude_x2 (tiled/reg, 2buf)
    ///@}

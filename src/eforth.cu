...
///@{
#define INT(f)    (__float2int_rn(f))           /**< cast float to int                       */
...
#define BOOL(f)   (ZERO(f) ? 0 : -1)            /**< default boolean representation          */
///@}
...
ForthVM::ForthVM(Istream *istr, Ostream *ostr, MMU *mmu0)
    ...
    int i = threadIdx.x;
    T4_DEBUG("D[%d]: dict=%p, mem=%p, vss=%p\n", i, dict, mmu.mem(0), mmu.vss(i));
}
///
...
__GPU__ void ForthVM::call(IU w) {
...
ForthVM::outer() {
    while (fin >> idiom) {                   /// loop throught tib
        MMU_DEBUG("%d>> %-10s => ", blockIdx.x, idiom);

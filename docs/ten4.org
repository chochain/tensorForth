...
** GPU, behaves like a co-processor or a DSP chip. 
** It has no OS, no string support, and runs its own memory. 
** Most of the available libraries are built for host instead of device
** So, to be interactive, a memory manager, IO, and syncing with CPU are things needed to be had. 
** need a Forth from scratch for a the GPU
  + Since GPUs have good compiler support nowadays, and
  + I've ported the latest [[https://github.com/chochain/eforth][eForth]] to lambda-based in C++, so C for now.
  + having *FP32* or *float32* as my basic data unit
  + *FP16*, or even fixed-point later
  + need to watch addressing and logic ops
